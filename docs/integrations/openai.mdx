---
title: OpenAI
description: Integrate with OpenAI's GPT models for chat, completions, and embeddings
---
...
OpenAI models support [streaming](/expression_language/streaming) the output of th model as it is generated. 
...
OpenAI has a [tool calling](/modules/model_io/models/chat_models/how_to/tools) (we use "tool calling" and "function calling" interchangeably here) API that lets you describe tools and their arguments, and have the model return a JSON object with a tool to invoke and the inputs to that tool. tool-calling is extremely useful for building tool-using chains and agents, and for getting structured outputs from models more generally.
