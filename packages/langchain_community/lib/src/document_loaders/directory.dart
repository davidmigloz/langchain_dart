...
class DirectoryLoader extends BaseDocumentLoader {
  ...
  Stream<Document> lazyLoad() async* {
    ...
    if (!directory.existsSync()) {
      throw ArgumentError('Directory does not exist: $filePath');
    }

    assert(glob.isNotEmpty, 'Glob pattern must not be empty');
    ...
    if (sampleSize > 0) {
      assert(
        ...
        'Sample size cannot be larger than total number of files',
      );
