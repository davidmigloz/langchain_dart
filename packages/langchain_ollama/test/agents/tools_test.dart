...
void main() {
  ...
  group('ChatOllamaToolsAgent tests', () {
    ...
    test('Test ChatOllamaToolsAgent with messages memory', () async {
      await testMemory(returnMessages: true);
    });

    test('Test ChatOllamaToolsAgent with string memory throws error', () async {
      expect(
  ...
  group('ChatOllamaToolsAgent LCEL equivalent test', () {
    ...
    final agent = Agent.fromRunnable(
      ...
      ).pipe(prompt).pipe(model).pipe(const OllamaToolsAgentOutputParser()),
      tools: [tool],
    );

    final executor = AgentExecutor(agent: agent);

    test('Test ChatOllamaToolsAgent LCEL equivalent', () async {
      final res = await executor.invoke({
        'input': 'What is 40 raised to the 0.43 power with 3 decimals?',
      });
      expect(res['output'], contains('4.885'));
    });
  });
}
