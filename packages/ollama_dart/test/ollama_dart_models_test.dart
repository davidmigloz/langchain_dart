  ...
  group(
    ...
    () {
      ...
      test('Test delete model', () async {
        ...
        expect(
          res1.models?.any((final m) => m.model == '$name:latest'),
          isTrue,
        );

        await client.deleteModel(
          request: const DeleteModelRequest(model: name),
        );

        final res2 = await client.listModels();
        expect(
          res2.models?.any((final m) => m.model == '$name:latest'),
          isFalse,
        );
      });
