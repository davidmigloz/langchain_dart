...
extension ToolResultBlockContentX on ToolResultBlockContent {
  ...
  String get text => map(
    text: (t) => t.value,
    blocks: (b) => b.value.whereType<TextBlock>().map((t) => t.text).join('\n'),
  );
...
extension BlockX on Block {
  ...
  String get text => mapOrNull(text: (text) => text.text) ?? '';

  /// Returns the image source of the block.
  ImageBlock? get image => mapOrNull(image: (image) => image);

  /// Returns the tool use block.
  ToolUseBlock? get toolUse => mapOrNull(toolUse: (toolUse) => toolUse);

  /// Returns the tool result block.
  ToolResultBlock? get toolResult =>
      mapOrNull(toolResult: (toolResult) => toolResult);
}
...
extension BlockDeltaX on BlockDelta {
  ...
  String get text =>
      map(textDelta: (text) => text.text, inputJsonDelta: (inputJson) => '');

  /// Returns the type of the block delta.
  String get inputJson => map(
    textDelta: (text) => '',
