  ...
  group('Streaming Architecture', () {
    ...
    group('text consolidation', () {
      ...
      test('text accumulation is monotonic', () async {
        ...
        await for (final chunk in agent.runStream('Tell me about the weather')) {
          accumulatedText += chunk.output;
          
          // Text should only grow, never shrink
          expect(accumulatedText.length, greaterThanOrEqualTo(previousLength));
          previousLength = accumulatedText.length;
        }
        
        expect(accumulatedText, isNotEmpty);
      });
    ...
    group('streaming edge cases', () {
      ...
      test('special characters in streaming', () async {
        ...
        final chunks = <String>[];
        await for (final chunk in agent.runStream('Echo: @#\$%^&*()')) {
          chunks.add(chunk.output);
        }
    ...
    group('stream termination and completion', () {
      ...
      test('stream can be canceled early', () async {
        ...
        var chunkCount = 0;
        await for (final chunk in agent.runStream('Write a long story')) {
          chunkCount++;
          if (chunkCount >= 3) {
            break; // Cancel early
          }
    ...
    group('provider-specific streaming behavior', () {
      ...
      test('streaming with different model types', () async {
        ...
        for (final model in models) {
          ...
          expect(hasContent, isTrue, reason: 'Model $model should stream content');
        }
      });
    });
    ...
    group('streaming error handling', () {
      test('network interruption handling', () async {
        ...
        expect(streamSucceeded || !streamSucceeded, isTrue);
      });
