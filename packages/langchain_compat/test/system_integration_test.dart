  ...
  group('System Integration', () {
    group('end-to-end workflows', () {
      test('complete agent conversation with tools', () async {
        ...
        var result = await agent.run('Hello! Can you help me test tools?', 
          history: history);
        expect(result.output, isNotEmpty);
        history.addAll(result.messages);
        
        // Turn 2: Use a tool
        result = await agent.run('Use string_tool with input "test"', 
          history: history);
        expect(result.output, isNotEmpty);
        history.addAll(result.messages);
    ...
    group('cross-provider workflows', () {
      ...
      test('provider-specific feature usage', () async {
        ...
        for (final testCase in testCases) {
          try {
            final agent = Agent(testCase['provider']! as String);
            final result = await agent.run(testCase['prompt']! as String);
            
            expect(result.output, isNotEmpty);
            // Feature-specific validation could be added here
          } on Exception catch (e) {
            // Provider-specific features may not always be available
            expect(e, isA<Exception>());
          }
        }
      });
    ...
    group('complex message handling', () {
      test('multipart message with tool execution', () async {
        ...
        final imageData = Uint8List.fromList([1, 2, 3, 4]); // Minimal image data
        final result = await agent.run(
    ...
    group('error recovery and resilience', () {
      ...
      test('recovery from network interruptions', () async {
        ...
        for (var i = 0; i < 3; i++) {
          futures.add(agent.run('Request $i').catchError((e) {
            // Convert errors to a result-like structure for analysis
      ...
      test('conversation continuation after errors', () async {
        ...
        try {
          ...
          // Add error context to history manually if needed
          expect(e, isA<Exception>());
        }
    ...
    group('performance and scaling', () {
      ...
      test('concurrent complex workflows', () async {
        ...
        final futures = workflows.map((workflow) => 
          workflow().catchError((e) => e)
        ).toList();
...
''';
    ...
    group('edge case integration', () {
      test('empty and minimal inputs', () async {
        ...
        for (final input in testCases) {
          try {
            ...
            // Edge case errors are acceptable
            expect(e, isA<Exception>());
          }
        }
      });
      ...
      test('very long workflow chains', () async {
        ...
        const longPrompt = 'Use string_tool with input "step1", '
